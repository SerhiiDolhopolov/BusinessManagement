...
async def send_message_to_other_admins(user_id: int, order_id: int, message: str):
    ...
    for admin_id, _, _ in users_DB.get_admins(user_id):
        try:
            ...
            keyboard_builder.row(InlineKeyboardButton(text=text, callback_data=f'or_ph|find|or_id:{order_id}'))
            await bot.send_message(admin_id, UI_phone_page.get_confirm_message_to_other_admin(username, role, order_id, message),
...
async def change_status(callback: CallbackQuery):
    ...
    for select_status_type in StatusType:
        ...
        if select_status_type == current_status_type:
            text = f'< {StatusManager.get_emoji(select_status_type)}{select_status_type} >'
        keyboard_builder.row(InlineKeyboardButton(text=text, 
                             callback_data=f'change_phone|set_status|id:{order_id}&type:{select_status_type.name}'))
    ...
    await callback.message.answer(text, reply_markup=keyboard_builder.as_markup(), parse_mode='HTML')
...
async def ask_comment(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard_builder = InlineKeyboardBuilder()
    keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_status_confirm_button(),
    ...
    message = await callback.message.answer(text, reply_markup=keyboard_builder.as_markup(), parse_mode='HTML')
...
async def edit_comment_message(chat_id: int, state: FSMContext, order_id: int, comment: str, new_status_type: StatusType):
    (order_id, order), (phone_id, phone) = orders_DB.get_order_phone(order_id)
...
async def ask_battery_status_message(chat_id: int, state: FSMContext, order_id: int):
    ...
    message = await bot.send_message(chat_id, text, reply_markup=keyboard_builder.as_markup())
    await phones_manager.save_need_to_delete_message(state, message.message_id)
...
async def filling_battery_status(message: Message, state: FSMContext):
    ...
    try:    
        ...
        keyboard_builder = InlineKeyboardBuilder()
        keyboard_builder.row(keyboards.get_back_button(f'or_ph|select|or_id:{order_id}'))
        text = UI_phone_page.get_confirm_change_battery_status(old_battery_status, phone.battery_status)
...
async def ask_memory_message(chat_id: int, order_id: int, page: int):
    ...
    keyboard_builder.row(*keyboards.get_pagination_row(page, f'change_phone|edit_memory|or_id:{order_id}&page', 
                                                       obj_count_on_page, all_count))
    keyboard_builder.row(keyboards.get_back_button(f'or_ph|select|or_id:{order_id}'))
    text = UI_phone_page.get_ask_change_memory(phone.memory)
    await bot.send_message(chat_id, text, reply_markup=keyboard_builder.as_markup())
...
async def ask_edit_defect(chat_id: int, state: FSMContext, order_id: int, page: int):
    ...
    keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, f'change_phone|edit_defect|or_id:{order_id}&page', 
                                                       obj_count_on_page, all_count))
    keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_defect_continue_button(), 
...
async def edit_defects_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(text=text, reply_markup=keyboard_builder.as_markup(), parse_mode='HTML')
    await send_message_to_other_admins(callback.from_user.id, order_id, text)
...
async def ask_solve_defect(chat_id: int, state: FSMContext, order_id: int, page: int):
    ...
    for defect_id, defect in defects_DB.get_defects(obj_count_on_page, obj_count_on_page * page).items():
        if defect in defects:
            keyboard_builder.add(InlineKeyboardButton(text=f"✅{defect}", 
                                          callback_data=f'change_phone|select_solve_defect|id:{defect_id}&page:{page}'))
    keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, f'change_phone|solve_defect|or_id:{order_id}&page', 
                                                       obj_count_on_page, len(defects)))
    keyboard_builder.row(keyboards.get_back_button(f'or_ph|select|or_id:{order_id}'))
...
async def solve_defect(callback: CallbackQuery, state: FSMContext):
    ...
    if phone.get_defects():
        keyboard_builder.add(InlineKeyboardButton(text=UI_phone_page.get_solve_defect_button(), 
...
async def ask_order_parametr_callback(callback:CallbackQuery, state:FSMContext):
    match = re.match(r'change_phone\|edit_(price_purchase|price_selling|charges)\|or_id:(\d+)', callback.data)
...
async def filling_order_parametr(message:Message, state:FSMContext, parametr:str):
    ...
    try:  
        if parametr == 'price_purchase':
            ...
            text = UI_phone_page.get_confirm_change_price_purchase(old_price_purchase, order.price_purchase)
        elif parametr == 'price_selling':
            ...
            text = UI_phone_page.get_confirm_change_price_selling(old_price_selling, order.price_selling)
        elif parametr == 'charges':
