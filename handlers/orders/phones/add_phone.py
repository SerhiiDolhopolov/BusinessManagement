...
async def ask_model_message(chat_id: int, page: int):
    ...
    for model_id, model in models.items():
        keyboard_builder.add(InlineKeyboardButton(text=model, callback_data=f'add_phone|select_model|id:{model_id}'))
    keyboard_builder = keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, 'add_phone|ask_model|page', obj_count_on_page, all_count))
    keyboard_builder.row(keyboards.get_back_button('menu|show|'))
    await bot.send_message(chat_id, UI_add_page.get_select_model(), reply_markup=keyboard_builder.as_markup())  
...
async def ask_color_message(chat_id: int, phone: Phone, page: int):
    ...
    colors = colors_DB.get_colors_by_id(models_colors_DB.get_colors_id(model_id), obj_count_on_page, obj_count_on_page * page)
    all_count = models_colors_DB.get_count_for_model(model_id)
    keyboard_builder = InlineKeyboardBuilder()
    for color_id, color in colors.items():
        keyboard_builder.add(InlineKeyboardButton(text=color, callback_data=f'add_phone|select_color|id:{color_id}'))
    keyboard_builder = keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, 'add_phone|ask_color|page', obj_count_on_page, all_count))
    keyboard_builder.row(keyboards.get_back_button('add_phone|ask_model|page:0'))
    await bot.send_message(chat_id, UI_add_page.get_select_color(), reply_markup=keyboard_builder.as_markup())
...
async def ask_memory_message(chat_id: int, phone: Phone, page: int):
    model_id = models_DB.get_model_id(phone.model)
    memories = memories_DB.get_memories_by_id(models_memories_DB.get_memories_id(model_id), 
                                              obj_count_on_page, obj_count_on_page * page)
    all_count = models_memories_DB.get_count_for_model(model_id)
    keyboard_builder = InlineKeyboardBuilder()
    for memory_id, memory in memories.items():
        keyboard_builder.add(InlineKeyboardButton(text=f"{memory}{MEMORY_DIMENSION}", 
                            callback_data=f'add_phone|select_memory|id:{memory_id}'))
    keyboard_builder = keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, 'add_phone|ask_memory|page', obj_count_on_page, all_count))
    keyboard_builder.row(keyboards.get_back_button('add_phone|ask_color|page:0'))
    await bot.send_message(chat_id, UI_add_page.get_select_memory(), reply_markup=keyboard_builder.as_markup())
...
async def ask_battery_status_message(chat_id: int, state: FSMContext, phone: Phone):
    ...
    message = await bot.send_message(chat_id, UI_add_page.get_ask_battery_status(), reply_markup=keyboard_builder.as_markup())
...
async def ask_defect(chat_id: int, state: FSMContext, phone: Phone, page: int):
    ...
    keyboard_builder = InlineKeyboardBuilder()
    for defect_id, defect in defects_DB.get_defects(obj_count_on_page, obj_count_on_page * page).items():
        keyboard_builder.add(InlineKeyboardButton(text=f"{'✅' if defect in phone.get_defects() else '❌'}{defect}", 
                                          callback_data=f'add_phone|select_defect|id:{defect_id}&page:{page}'))
    keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, 'add_phone|select_defect|page', obj_count_on_page, all_count))
    keyboard_builder.row(InlineKeyboardButton(text=UI_add_page.get_continue_button(), callback_data='add_phone|filling_purchase_price|'))
    keyboard_builder.row(keyboards.get_back_button('add_phone|filling_battery_status|'))
    message = await bot.send_message(chat_id, UI_add_page.get_select_defects(phone), reply_markup=keyboard_builder.as_markup())
    await phones_manager.save_need_to_delete_message(state, message.message_id)
...
async def ask_order_purchase_price_message(chat_id: int, state: FSMContext):
    ...
    message = await bot.send_message(chat_id, UI_add_page.get_ask_price_purchase(), reply_markup=keyboard_builder.as_markup())
    await phones_manager.save_need_to_delete_message(state, message.message_id)
...
async def ask_order_comment(message: Message, state: FSMContext):
    await phones_manager.delete_ask_messages(message.from_user.id, state)
    keyboard_builder = InlineKeyboardBuilder()
    keyboard_builder.row(InlineKeyboardButton(text=UI_add_page.get_confirm_button(), callback_data='add_phone|confirm|'))
    ...
    message = await message.answer(UI_add_page.get_ask_comment(), reply_markup=keyboard_builder.as_markup())
    await phones_manager.save_need_to_delete_message(state, message.message_id)
...
async def confirm_message(chat_id: int, state: FSMContext, comment: str):
    ...
    keyboard_builder = InlineKeyboardBuilder()
    keyboard_builder.row(InlineKeyboardButton(text=UI_add_page.get_order_button(order_id), 
                                              callback_data=f'or_ph|find|or_id:{order_id}'))
    keyboard_builder.row(keyboards.get_back_button('menu|show|'))
    await bot.send_message(chat_id, text, parse_mode='HTML', reply_markup=keyboard_builder.as_markup())
    user_username, user_role = users_DB.get_user(chat_id)
    for admin_id, _, _ in users_DB.get_admins(chat_id):
        await bot.send_message(admin_id, UI_add_page.get_confirm_message_to_other_admin(user_username, user_role, text),
                               parse_mode='HTML', reply_markup=keyboard_builder.as_markup())
...
async def add_random_phones_to_database(count: int):
    ...
    for i in range(count):
        ...
        memories = list(memories_DB.get_memories_by_id(models_memories_DB.get_memories_id(model_id)).values())
        colors = list(colors_DB.get_colors_by_id(models_colors_DB.get_colors_id(model_id)).values())
        ...
        for status_type in random.choices(population=[
            ...
            status.date_time = datetime.now(TIMEZONE) + (timedelta(seconds=add_seconds, minutes=5) 
                                                    if status_type == StatusType.AVAILABLE
                                                    else 
                                                    timedelta(seconds=add_seconds, minutes=20))
            user_id = random.choice(users_ids)
            statuses_DB.add_status(user_id, order_id, status)
