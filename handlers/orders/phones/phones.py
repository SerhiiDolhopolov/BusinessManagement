...
async def get_phones(callback: CallbackQuery, state: FSMContext):
    ...
    status_type = None if page_status_type in ('all', 'my') or re.match(r'(.+)defect_(\w+)', page_status_type) \
    ...
    if page_status_type == 'my':
        orders_phones = statuses_DB.get_orders_by_user(callback.from_user.id, obj_count_on_page, page * obj_count_on_page)
        ...
        orders_phones = statuses_DB.get_orders_by_defect(status_type, defect, obj_count_on_page, page * obj_count_on_page)
        ...
        orders_phones = orders_DB.get_orders_phones(status_type, obj_count_on_page, page * obj_count_on_page)
    ...
    keyboard_builder.row(*keyboards.get_pagination_row(page, f'or_ph_{page_status_type}|show|page', 
...
async def select_phone_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await select_phone(callback.from_user.id, order_id, f'or_ph_{page_status_type}|show|page:{page}')
...
async def select_phone(chat_id: int, order_id: int, callback_back_button: str = None):
    ...
    if role in (Role.ADMIN, Role.MANAGER):
        keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_change_price_purchase_button(),
                            callback_data=f'change_phone|edit_price_purchase|or_id:{order_id}'),
                            InlineKeyboardButton(text=UI_phone_page.get_change_charges_button(),
                            callback_data=f'change_phone|edit_charges|or_id:{order_id}'),
                            InlineKeyboardButton(text=UI_phone_page.get_change_price_selling_button(),
                            callback_data=f'change_phone|edit_price_selling|or_id:{order_id}'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_change_memory_button(),
                            callback_data=f'change_phone|edit_memory|or_id:{order_id}&page:0'),
                            InlineKeyboardButton(text=UI_phone_page.get_change_battery_button(),
                            callback_data=f'change_phone|edit_battery_status|or_id:{order_id}'),
                            InlineKeyboardButton(text=UI_phone_page.get_change_defects_button(), 
                            callback_data=f'change_phone|edit_defect|or_id:{order_id}&page:0'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_change_status_button(), 
                            callback_data=f'change_phone|status|id:{order_id}'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_solve_defect_button(), 
                            callback_data=f'change_phone|solve_defect|or_id:{order_id}&page:0'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_phone_page.get_history_button(), 
    ...
    await bot.send_message(chat_id, phone_information, parse_mode='HTML', reply_markup=keyboard_builder.as_markup())
...
async def show_select_defect(callback: CallbackQuery):
    ...
    status_type = StatusType.WAITING_FOR_SPARES if what_status_type == 'spares' else StatusType.WAITING_FOR_REPAIRS

    defects_group = statuses_DB.get_defect_groups_orders(status_type, obj_count_on_page, page * obj_count_on_page)
    ...
    for defect, count in defects_group.items():
        keyboard_builder.add(InlineKeyboardButton(text=f"{defect}:{count}", 
                                                  callback_data=f"or_ph_{what_status_type}_defect_{defect}|show|page:0"))
    keyboard_builder.adjust(2)
    keyboard_builder.row(*keyboards.get_pagination_row(page, f"or_ph_{what_status_type}_defects|show_categories|page", obj_count_on_page, all_count))
