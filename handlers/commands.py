    ...
    if not users_DB.is_user_exist(message.from_user.id):
        ...
        for role in Role:
            if role != current_role and role != Role.ADMIN:
                keyboard_builder.add(InlineKeyboardButton(text=role, 
        ...
        for admin_id, _, _ in users_DB.get_admins(message.from_user.id):
            await bot.send_message(admin_id, UI_menu.get_user_registration_select_role(message.from_user.username), 
...
async def command_menu(chat_id: int):
    ...
    if role is Role.USER:
        for _, admin_username, _ in users_DB.get_admins():
            keyboard_builder.row(InlineKeyboardButton(text=UI_menu.get_chat_with_admin_button(admin_username), 
                                                      url=f'https://t.me/{admin_username}'))

    if role in (Role.ADMIN, Role.MANAGER, Role.COURIER):
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_my_phones_button(), 
                                                  callback_data='or_ph_my|show|page:0'),
                             InlineKeyboardButton(text=UI_main_page.get_add_button(), 
                                                  callback_data='add_phone|ask_model|page:0'))

    keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_available_button(), 
                                               callback_data=f'or_ph_{StatusType.AVAILABLE.name}|show|page:0'))

    if role in (Role.ADMIN, Role.MANAGER):
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_phones_button(), 
                                                  callback_data='phone_status_menu|show|'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_need_spares_button(), 
                                                  callback_data='or_ph_spares_defects|show_categories|page:0'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_need_repairs_button(), 
                                                  callback_data='or_ph_repairs_defects|show_categories|page:0'))

    if role == Role.ADMIN:
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_users_button(), callback_data='users|show|page:0'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_couriers_button(), callback_data='users|show_couriers|page:0'),
                             InlineKeyboardButton(text=UI_main_page.get_managers_button(), callback_data='users|show_managers|page:0'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_admin_panel_button(), callback_data='admin_menu|show|'))
        keyboard_builder.row(InlineKeyboardButton(text=UI_main_page.get_statistics_button(), callback_data='statistics|show|')) 

    await bot.send_message(chat_id, UI_main_page.get_info(role), reply_markup=keyboard_builder.as_markup())
...
async def command_ask_role(message: Message, state: FSMContext):
    try:
        await bot.delete_message(message.from_user.id, int((await state.get_data())['message_for_delete']))
    ...
    try:
        ...
        if (users_DB.is_user_exist(user_id)):
            ...
            await message.answer(UI_commands.get_select_role(username),
...
async def get_backup() -> tuple[FSInputFile, str]:
    ...
    message = UI_commands.get_backup_info(statuses_count, price_selling_sum, profit_money_sum, price_purchase_sum, charges_sum)
