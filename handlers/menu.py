...
async def show_admin_menu(callback: CallbackQuery):
    ...
    keyboard_builder.row(InlineKeyboardButton(text=UI_admin_page.get_models_button(), callback_data='models|show|page:0'),
                InlineKeyboardButton(text=UI_admin_page.get_colors_button(), callback_data='colors|show|page:0'))
    keyboard_builder.row(InlineKeyboardButton(text=UI_admin_page.get_defects_button(), callback_data='defects|show|page:0'),
                InlineKeyboardButton(text=UI_admin_page.get_memory_button(), callback_data='memories|show|page:0'))
    keyboard_builder.row(keyboards.get_back_button('menu|show|'))
    await callback.message.answer(UI_admin_page.get_info(), reply_markup=keyboard_builder.as_markup())
...
async def show_phones_status_menu(callback: CallbackQuery):
    keyboard_builder = InlineKeyboardBuilder()
    keyboard_builder.row(InlineKeyboardButton(text=UI_phones_page.get_status_button(), callback_data='or_ph_all|show|page:0'))
    i = 0
    for status in StatusType:
        ...
        i += 1
        strategy(InlineKeyboardButton(text=UI_phones_page.get_status_button(status), callback_data=f'or_ph_{status.name}|statuses|page:0'))
    keyboard_builder.row(keyboards.get_back_button('menu|show|'))
    await callback.message.answer(UI_phones_page.get_select_status(), reply_markup=keyboard_builder.as_markup())
