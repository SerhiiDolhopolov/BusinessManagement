...
async def show_models_page(message: Message, page: int):
    ...
    await message.answer(
        UI_admin_page.get_models_info(),
        reply_markup=get_show_objects_keyboard(
            'models', page, models, all_count, obj_count_on_page
        )
    )
...
async def show_model_colors_page(message: Message, model_id: int, page: int):
    model = models_DB.get_model(model_id)
    colors = models_colors_DB.get_colors_with_flag(
        model_id, obj_count_on_page, page * obj_count_on_page
    )
    all_count = colors_DB.get_all_count()
    await message.answer(
        UI_admin_page.get_ask_select_colors(model),
        reply_markup=get_model_show_objects_keyboard(
            model_id, 'colors', page, colors, all_count, obj_count_on_page
        )
    )
...
async def show_model_memories_page(message: Message, model_id: int, page: int):
    model = models_DB.get_model(model_id)
    memories = models_memories_DB.get_memories_with_flag(
        model_id, obj_count_on_page, page * obj_count_on_page
    )
    all_count = memories_DB.get_all_count()
    await message.answer(
        UI_admin_page.get_ask_select_memories(model),
        reply_markup=get_model_show_objects_keyboard(
            model_id, 'memories', page, memories, all_count, obj_count_on_page
        )
    )
