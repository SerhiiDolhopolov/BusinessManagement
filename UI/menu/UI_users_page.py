...
def get_users_text(users: list[int, str, Role, bool]) -> str:
    return __users_page.get('users').format(users=__get_users_text(users))

def get_couriers_text(users: list[int, str, Role, bool]) -> str:
    return __users_page.get('couriers').format(users=__get_users_text(users))

def get_managers_text(users: list[int, str, Role, bool]) -> str:
    return __users_page.get('managers').format(users=__get_users_text(users))

def __get_users_text(users: list[int, str, Role, bool]) -> str:
    return '\n'.join([f"{RoleManager.get_emoji(role)}<code>{html.escape(str(user_id))}</code>: @{user_username}"
