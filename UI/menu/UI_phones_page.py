...
def get_info(status_type: StatusType = None) -> str:
    ...
    return __phones_page.get('info').format(emoji=emoji, status=status_type) if status_type else __phones_page.get('all_phones_info')

def get_status_button(status_type: StatusType = None) -> str:
    """If status_type == None return all phones info"""
    emoji = StatusManager.get_emoji(status_type)
    return __phones_page.get('status_button').format(emoji=emoji, status=status_type) if status_type else __phones_page.get('all_phones_status_button')

def get_select_status() -> str:
    return __phones_page.get('select_status')

def get_phone_button(order: Order, phone: Phone) -> str:
    status_emoji = StatusManager.get_emoji(order.status.status_type) if order.status.status_type else ''
    ...
    return __phones_page.get('phone_button').format(status_emoji=status_emoji, title=title, price_selling=price_selling, money_emoji=money_emoji)
