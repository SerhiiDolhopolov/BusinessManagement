...
def get_confirm_message(order: Order, order_id: int, phone: Phone) -> str:
    ...
    title = f"<code>{html.escape(f"{phone.model} {phone.memory}{MEMORY_DIMENSION} | {phone.color}")}</code>"
    ...
    return __add_page.get('confirm_message').format(order_id=order_id, title=title, battery_status=battery_status,
                                            defects=defects, price_purchase=price_purchase, comment=comment,
                                            date_time=date_time)

def get_confirm_message_to_other_admin(user_username: str, user_role: Role, text: str) -> str:
    emoji_user = RoleManager.get_emoji(user_role)
    return __add_page.get('confirm_message_to_other_admin').format(emoji_user=emoji_user, username=user_username, text=text)
