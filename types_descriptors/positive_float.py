...
class PositiveFloat:
    ...
    def __init__(
        self, can_be_none: bool = False, min_value: int = None, max_value: int = None
    ):
    ...
    def __set__(self, instance, value: float):
        if self.__can_be_none and value is None:
            ...
            if value < 0:
                raise ValueError(UI_descriptors.get_positive_float_less_than_0_error(value=value))
            if type(self.__min) is float and value < self.__min:
                raise ValueError(
                    UI_descriptors.get_positive_float_min_error(
                        value=value, min_value=self.__min
                    )
                )
            if type(self.__max) is float and value > self.__max:
                raise ValueError(
                    UI_descriptors.get_positive_float_max_error(
                        value=value, max_value=self.__max
                    )
                )
