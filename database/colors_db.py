...
class ColorsDB(DB):
    ...
    def get_color(self, color_id: int) -> str:
        answer = self._fetchone(
            f"SELECT DISTINCT {self.COLOR} FROM {self.TABLE_NAME} WHERE {self.ID} = {color_id};"
        )
        return answer[0]

    def get_color_id(self, color: str) -> int:
        answer = self._fetchone(
            f"SELECT DISTINCT {self.ID} FROM {self.TABLE_NAME} WHERE {self.COLOR} = {DB.sql(color)};"
        )
        return answer[0]
    ...
    def get_colors(self, limit: int = 0, offset: int = 0) -> dict[int, str]:
        ...
        answer = self._fetchall(
            f"""SELECT DISTINCT {self.ID}, {self.COLOR} 
    ...
    def get_colors_by_id(
        self, color_id_list: list[int] = None, limit: int = 0, offset: int = 0
    ) -> dict[int, str]:
        """RETURNS {color_id:color}"""
        if not color_id_list:
            return dict()
        answer = self._fetchall(
            f"""SELECT DISTINCT {self.ID}, {self.COLOR} FROM {self.TABLE_NAME} 
