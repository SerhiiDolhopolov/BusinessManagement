...
class MemoriesDB(DB):
    ...
    def get_memory(self, memory_id: int) -> int:
        answer = self._fetchone(f"SELECT DISTINCT {self.MEMORY} FROM {self.TABLE_NAME} WHERE {self.ID} = {memory_id};")
        return answer[0] 
    ...
    def get_memories(self, limit: int = 0, offset: int = 0) -> dict[int, int]:
        ...
        answer = self._fetchall(f"""SELECT DISTINCT {self.ID}, {self.MEMORY} 
                                FROM {self.TABLE_NAME} 
                                ORDER BY {self.MEMORY} ASC
                                {f"LIMIT {limit} OFFSET {offset}" if limit else ''};""")
        return {line[0]: line[1] for line in answer}

    def get_memories_by_id(self, memory_id_list: list[int] = None, limit: int = 0, offset: int = 0) -> dict[int, int]:
        """RETURNS {memory_id:memory}"""
        if not memory_id_list:
            return dict()
        answer = self._fetchall(f"""SELECT DISTINCT {self.ID}, {self.MEMORY} FROM {self.TABLE_NAME} 
