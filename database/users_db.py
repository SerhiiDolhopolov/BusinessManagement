...
class UsersDB(DB):
    ...
    def __get_users_request(
        self, where: str = '', limit: int = 0, offset: int = 0
    ) -> list[tuple[int, str, 'Role']]:
        """RETURNS list[(telegram_id, username, role)]"""
        users = self._fetchall(
            f"""SELECT {self.ID}, {self.USERNAME}, {self.ROLE} FROM {self.TABLE_NAME} 
            {where} 
    ...
    def get_users_by_role(
        self, role: Role, limit: int = 0, offset: int = 0
    ) -> list[(int, str, Role)]:
        """RETURNS list[(telegram_id, username, role)]"""
    ...
    def get_admins(self, except_telegram_id: int = None) -> list[(int, str, Role)]:
        """RETURNS list[(telegram_id, username, role)]"""
        return self.__get_users_request(
            f"""WHERE {self.ROLE} IN ('{Role.ADMIN}') 
            {f'AND {self.ID} != {except_telegram_id}' if except_telegram_id else ''}"""
        )

    def get_managers_and_higher(
        self, except_telegram_id: int = None
    ) -> list[(int, str, Role)]:
        """RETURNS list[(telegram_id, username, role)]"""
        return self.__get_users_request(
            f"""WHERE {self.ROLE} IN ('{Role.ADMIN}', '{Role.MANAGER}') 
            {f'AND {self.ID} != {except_telegram_id}' if except_telegram_id else ''}"""
        )
    ...
    def get_user(self, telegram_id: int) -> tuple[str, Role]:
        ...
        user = self._fetchone(
            f"SELECT {self.USERNAME}, {self.ROLE} FROM {self.TABLE_NAME} WHERE {self.ID} = {telegram_id}"
        )
