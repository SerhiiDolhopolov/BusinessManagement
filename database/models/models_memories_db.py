...
class ModelsMemoriesDB(DB):
    ...
    def is_related(self, model_id: int, memory_id: int) -> bool:
        answer = self._fetchone(
            ...
            f"AND {self.MEMORY_ID} = {memory_id};"
        )
    ...
    def delete_relation(self, model_id: int, memory_id: int):
        self._delete_data(
            self.TABLE_NAME,
            f"WHERE {self.MODEL_ID} = {model_id} AND {self.MEMORY_ID} = {memory_id}"
        )

    def get_count_for_model(self, model_id: int) -> int:
        answer = self._fetchone(
            f"""SELECT COUNT(DISTINCT {self.MEMORY_ID})
    ...
    def get_memories_id(self, model_id: int) -> list[int]:
        answer = self._fetchall(
            f"SELECT DISTINCT {self.MEMORY_ID} FROM {self.TABLE_NAME} WHERE {self.MODEL_ID} = {model_id};"
        )
        return [] if not answer else [memory_id[0] for memory_id in answer]

    def get_memories_with_flag(
        self, model_id: int, limit: int = 0, offset: int = 0
    ) -> dict[int, (int, bool)]:
        """RETURNS { color_id: (memory, is_using_by_model) }"""
        answer = self._fetchall(
            f"""SELECT DISTINCT {MemoriesDB.ID}, {MemoriesDB.MEMORY}, CASE WHEN
            {MemoriesDB.ID} IN 
