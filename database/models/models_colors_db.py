...
class ModelsColorsDB(DB):
    ...
    def is_related(self, model_id:int, color_id:int) -> bool:
        answer = self._fetchone(f"SELECT COUNT(*) FROM {self.TABLE_NAME} WHERE {self.MODEL_ID} = {model_id} AND {self.COLOR_ID} = {color_id};")
    ...
    def delete_relation(self, model_id:int, color_id:int):
        self._delete_data(self.TABLE_NAME, f"WHERE {self.MODEL_ID} = {model_id} AND {self.COLOR_ID} = {color_id}")

    def get_count_for_model(self, model_id:int) -> int:
        answer = self._fetchone(f"""SELECT COUNT(DISTINCT {self.COLOR_ID})
                                FROM {self.TABLE_NAME}
                                WHERE {self.MODEL_ID} = {model_id};""")
        return None if not answer else int(answer[0])

    def get_colors_id(self, model_id:int) -> list[int]:
        answer = self._fetchall(f"SELECT DISTINCT {self.COLOR_ID} FROM {self.TABLE_NAME} WHERE {self.MODEL_ID} = {model_id};")
        return [] if not answer else [color_id[0] for color_id in answer]

    def get_colors_with_flag(self, model_id:int, limit:int = 0, offset:int = 0) -> dict[int:(str, bool)]:
        """RETURNS { color_id: (color, is_using_by_model) }"""
        answer = self._fetchall(f"""SELECT DISTINCT {ColorsDB.ID}, {ColorsDB.COLOR}, CASE WHEN
                                {ColorsDB.ID} IN 
