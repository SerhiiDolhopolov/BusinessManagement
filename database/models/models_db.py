...
class ModelsDB(DB):
    ...
    def get_model(self, model_id: int) -> str:
        answer = self._fetchone(
            f"SELECT {self.MODEL} FROM {self.TABLE_NAME} WHERE {self.ID} = {model_id};"
        )
        return answer[0]

    def get_model_id(self, model: str) -> int:
        answer = self._fetchone(
            f"SELECT {self.ID} FROM {self.TABLE_NAME} WHERE {self.MODEL} = {DB.sql(model)};"
        )
