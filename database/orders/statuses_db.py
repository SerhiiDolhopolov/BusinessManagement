...
class StatusesDB(DB):
    ...
    def get_current_status_type(self, order_id: int) -> StatusType:
        answer = self._fetchone(
            f"""SELECT {self.STATUS_TYPE}, MAX({self.DATE_TIME})
    ...
    def get_count_orders_by_user(self, user_id: int) -> int:
        answer = self._fetchone(
            f"""SELECT COUNT(DISTINCT({StatusesDB.ORDER_ID}))
    ...
    def get_orders_by_user(
        ...
        answer = self._fetchall(
            f"""SELECT {PhonesDB.ID}, {PhonesDB.MODEL}, {PhonesDB.COLOR}, 
    ...
    def get_orders_by_defect(
        self, status_type: StatusType, defect: str, limit: int = 0, offset: int = 0
    ) -> list[(int, Order), (int, Phone)]:
    ...
    def get_count_orders_with_current_status(
        self, status_type: StatusType = None
    ) -> int:
        answer = self._fetchone(
            f"""WITH LatestStatusesS AS (
    ...
    def get_count_defect_groups_orders(self, status_type: StatusType) -> int:
        ...
        answer = self._fetchone(
            f"""SELECT COUNT(DISTINCT {PhonesDefectsDB.defect}) FROM {OrdersDB.TABLE_NAME}
            JOIN {PhonesDefectsDB.TABLE_NAME} USING ({PhonesDefectsDB.PHONE_ID})
    ...
    def get_defect_groups_orders(
        self, status_type: StatusType, limit: int = 0, offset: int = 0
    ) -> dict[str, int]:
        """RETURNS {defect, count} with selected status type"""
        answer = self._fetchall(
            f"""SELECT {PhonesDefectsDB.defect}, COUNT({OrdersDB.ID})
            FROM {OrdersDB.TABLE_NAME}
            JOIN {PhonesDefectsDB.TABLE_NAME} USING ({PhonesDefectsDB.PHONE_ID})
    ...
    def get_statuses_ascending(self, order_id: int) -> dict[int, (int, Status)]:
        ...
        answer = self._fetchall(
            f"""SELECT {self.ID}, {self.USER_ID}, {self.STATUS_TYPE}, {self.COMMENT}, {self.DATE_TIME}
            FROM {self.TABLE_NAME}
            WHERE {self.ORDER_ID} = {DB.sql(order_id)}
    ...
    def get_date_time_now(self) -> datetime:
        ...
        return (
            f'{"{:04d}".format(date_time_now.year)}-'
            f'{"{:02d}".format(date_time_now.month)}-'
            f'{"{:02d}".format(date_time_now.day)}'
        )