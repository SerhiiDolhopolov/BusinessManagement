...
logging_level = getattr(logging, getenv('LOGGING_LEVEL').upper(), logging.INFO)
...
def get_backup_name() -> str:
    return TEMP_PATH / f"backup_{datetime.now(TIMEZONE).replace(microsecond=0).strftime(DATE_TIME_FORMAT)}.db"
