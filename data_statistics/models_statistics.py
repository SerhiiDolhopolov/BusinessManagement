...
class ModelsStatistics(Statistics):
    PATH_DIR = TEMP_PATH / Path('models_diagram')

    def __init__(self):
        df = self._read_sql(parse_dates=[StatusesDB.DATE_TIME])
        self._clear_dataframe(df)
        self._add_dates_from_date_time(df, 'date_time')
        data = self._get_grouped_data(df, ['date', 'model'])
        self._add_trace_color(data, 'model')
...
SELECT {OrdersDB.ID}, {OrdersDB.PRICE_PURCHASE}, {OrdersDB.PRICE_SELLING}, {OrdersDB.CHARGES}, 
    ...
    def _get_figure(self):
        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=('Income Sum', 'Models Count'),
            shared_xaxes=True
        )
